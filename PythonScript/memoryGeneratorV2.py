...
def shipDimension (direction):
    if(direction == 0):
        ...
        for i in (2,3,4,5):
            ...
            print("\t\t\t\t\t\tif("+var_memory+"["+var_in_x+"+4'd"+str(l)+"]["+var_in_y+"] == 5'd1 //se è pre allocata")
            while l < (i-1):
                print("\t\t\t\t\t\t\t\t&& "+var_memory+"["+var_in_x+"+4'd"+str(l+1)+"]["+var_in_y+"] == 5'd1")
...
print ("reg [3:0] "+var_memory+" [0:9][0:9];")
